...
class ImageTar():
  ...
  def get_path_to_layer_contents(self, layer_num):
    ...
    if not os.path.isdir(path):
      ...
      layer_tar = tarfile.open(self.contents_folder + self.layers[layer_num])
      try:
...
def compdirs(left, right, diff=None, path=""):
  ...
  for funny_file in diff.funny_files:
    if not (os.path.islink(left + "/" + path + funny_file) and
            os.path.islink(right + "/" + path + funny_file) and
            os.readlink(left + "/" + path + funny_file) == os.readlink(
                right + "/" + path + funny_file)):
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      "-d",
      "--max_differences",
      help="stops after MAX_DIFFERENCES different layers are found",
      type=int,
      default=float("inf"))
  parser.add_argument(
      "-l",
